...
test('shorthand uses current scenery names, explicit labels override, and emphasis remains intact',()=>{
    ...
    assert.deepEqual(game.parseExamineLinks('The [[lamp]], [[glow|lamp]] and **wall**.'),[
        {type:'text',text:'The '},{type:'examine',text:'street lamp',sceneryKey:'lamp'},
        {type:'text',text:', '},{type:'examine',text:'glow',sceneryKey:'lamp'},
        {type:'text',text:' and '},{type:'accent',text:'wall'},{type:'text',text:'.'},
    ]);
    assert.equal(game.parseExamineLinks('[[lamp]]',game.state.rooms.hall.scenery)[0].text,'oil lamp');
    assert.equal(game.parseExamineLinks('[[wall]]')[0].text,'Wall');
    assert.equal(game.parseExamineLinks('[[unknown]]')[0].text,'unknown');
    assert.equal(game.getExamineLinkTarget('unknown',game.state.rooms.gallery.scenery),null);
...
test('renamed scenery and saved names resolve afresh; popup titles are independent',()=>{
    const game=createGame(world());
    game.state.rooms.gallery.scenery.lamp.name='iron lamp';
    ...
    assert.equal(restored.dispatch({type:'examineScenery',target:'gallery:lamp'}).messages[0].args[1],'Street Lamp');
    delete restored.state.rooms.gallery.scenery.lamp.title;
    assert.equal(restored.dispatch({type:'examineScenery',target:'gallery:lamp'}).messages[0].args[1],'iron lamp');
});
