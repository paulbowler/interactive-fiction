...
Reusable JavaScript packages for standalone point-and-click games. **Version 1.0.0 · MIT · Node.js 22+ tooling · No runtime dependencies.**
...
A game supplies its world, story controller and assets. Neither package imports a particular game. The King's Diamond has moved to a separate sibling project; its prose, images, rules and legacy reference implementation are not in this repository or these packages.
...
my-game/
  ...
  data/game.json           # declarative world and prose
...
Ordinary taking, dropping, containers, locks, inventory and movement need world properties, not bespoke scripts. The [API guide](docs/api.md) describes the retained world schema and supported actions.

## Install or upgrade a release

Packages are distributed as npm-compatible archives on [GitHub Releases](https://github.com/paulbowler/interactive-fiction/releases). No npm registry account is needed to consume them.

```sh
npm install --save-exact https://github.com/paulbowler/interactive-fiction/releases/download/v1.0.0/paulbowler-if-engine-1.0.0.tgz https://github.com/paulbowler/interactive-fiction/releases/download/v1.0.0/paulbowler-if-browser-1.0.0.tgz
```
...
The game-level King's Diamond suite is maintained with that game. A platform change should also run its headless and Playwright tests before release; see [release procedure](docs/releases.md).
...
V1 is synchronous and single-player. It supports the interactions extracted from the working game, not a general conversation or natural-language parser system. The default browser view mounts once per document. Game scripts and prose are trusted application content; see [security boundaries](SECURITY.md).
